...
def summary():
    ...
    return BaseSummary(
        ...
        summary_id="123",
        summary={"key": {"structured_summary": [StructuredSummary(**item) for item in items]}},
...
async def test_summary_type(summary):
    ...
    assert result[0]["file"].getvalue() == "Lorem ipsum dolor sit amet. Consectetur adipiscing elit."
    assert result[0]["title"] == "narrative_key_123"
    assert result[0]["alt_text"] == "Summary 123 of type narrative with a length of key."
...
async def test_multiple_summaries():
    ...
    summary = BaseSummary(
        ...
        summary_id="123",
        summary={
            "key1": {"structured_summary": [StructuredSummary(**{"summary": "Lorem ipsum dolor sit amet."})]},
            "key2": {"structured_summary": [StructuredSummary(**{"summary": "Consectetur adipiscing elit."})]}
        },
        summary_type="narrative",
    )
    result = await format_files_to_upload(summary)
    ...
    assert result[0]["alt_text"] == "Summary 123 of type narrative with a length of key1."
    assert result[0]["snippet_type"] == "text"
    ...
    assert result[1]["alt_text"] == "Summary 123 of type narrative with a length of key2."
